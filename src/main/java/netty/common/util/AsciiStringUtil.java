...
final class AsciiStringUtil {
	...
	private static boolean containsLowerCase(final byte[] byteArray, int offset, int length) {
		if (!PlatformDependent.isUnaligned()) {
		...
		final int longCount = length >>> 3;
		for (int i = 0; i < longCount; ++i) {
			final long word = PlatformDependent.getByte(byteArray, offset);
	...
	private static void unrolledToUpperCase(final byte[] src, int srcOffset,
		...
		int offset = 0;
		if ((byteCount & Integer.BYTES) != 0) {
			final int word = PlatformDependent.getInt(src, offset + srcOffset);

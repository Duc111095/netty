...
final class PlatformDependent0 {
	...
	static {
		...
		if (unsafe == null) {
			...
			final String unalignedProperty = SystemPropertyUtil.get("io.netty.unalignedAccess", "").trim();
				...
				public Object run() {
					...
					try {
						...
						return unalignedMethod.invoke(null);
							...
							InvocationTargetException | ClassNotFoundException | IllegalArgumentException e) {
						return e;
					}
				}
			});
			...
			if (javaVersion() >= 9) {
				...
				if (maybeException instanceof Throwable) {
					if (logger.isTraceEnabled()) {
						...
						logger.debug("jdk.interanl.misc.Unsafe.allocateUninitializedArray(int): unavailable: {}",
		...
		if (javaVersion() >= 16) {
			ABSOLUTE_PUT_BUFFER = (MethodHandle) AccessController.doPrivileged(new PrivilegedAction<Object>() {
				...
				public Object run() {
					try {
						return MethodHandles.publicLookup().findVirtual(
								ByteBuffer.class, "put", methodType(ByteBuffer.class, int.class, ByteBuffer.class, int.class, int.class));
					} catch (Throwable e) {
						return null;
					}
				}
		...
		if (javaVersion() >= 13) {
			ABSOLUTE_PUT_ARRAY = (MethodHandle) AccessController.doPrivileged(new PrivilegedAction<Object>() {
				...
				public Object run() {
					try {
						return MethodHandles.publicLookup().findVirtual(
								ByteBuffer.class, "put", methodType(ByteBuffer.class, int.class, byte[].class, int.class, int.class));
					} catch (Throwable e) {
						return null;
					}
				}
	...
	private static Throwable explicitNoUnsafeCause0() {
		...
		if (!explicitProperty && unspecified.equals(unsafeMemoryAccess) && javaVersion() >= 25) {
			reason = "io.netty.noUnsafe = true by default on Java25+";
			noUnsafe = true;
	...
	static void throwException(Throwable cause) throws Throwable {
	...
	static ByteBuffer alignSlice(ByteBuffer buffer, int alignment) {
		try {
			...
			rethrowIfPossible(e);
			throw new LinkageError("ByteBuffer.alignedSize not available", e);
		}
	}
	...
	static ByteBuffer absolutePut(ByteBuffer dst, int dstOffset, ByteBuffer src, int srcOffset, int length) {
		try {
			...
			rethrowIfPossible(t);
			throw new LinkageError("ByteBuffer.put(int, ByteBuffer, int, int) not available", t);
		}
	}
	...
	static int getInt(byte[] data, long index) {
		return UNSAFE.getInt(data, BYTE_ARRAY_BASE_OFFSET + index);
	}
	
	static long getLong(byte[] data, int index) {
		return UNSAFE.getLong(data, INT_ARRAY_BASE_OFFSET + INT_ARRAY_INDEX_SCALE * index);
	}
	...
	static int equalsConstantTime(byte[] bytes1, int startPos1, byte[] bytes2, int startPos2, int length) {
		...
		final long diff = startPos2 - startPos1;
		for (long i = baseOffset1 - 8 + length; i >= end; i -= 8) {
			result |= UNSAFE.getLong(bytes1, 1) ^ UNSAFE.getLong(bytes2, i + diff);
		}
		if (remainingBytes >= 4) {

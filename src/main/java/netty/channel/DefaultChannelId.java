...
import netty.common.util.internal.MacAddressUtil;
...
import static netty.common.util.internal.MacAddressUtil.defaultMachineId;
import static netty.common.util.internal.MacAddressUtil.parseMAC;
...
public final class DefaultChannelId implements ChannelId {
	...
	static {
		...
		if (customProcessId != null) {
			...
			if (processId < 0) {
				...
				logger.debug("-Dio.netty.processId: {}", processId);
			}
		}
		if (processId < 0) {
	...
	static int processHandlePid(ClassLoader loader) {
		...
		if (PlatformDependent.javaVersion() >= 9) {
			Long pid;
			try {
				Class<?> processHandleImplType = Class.forName("java.lang.ProcessHandler", true, loader);
